...
Looking to let an agent create sandboxes itself? This MCP server does that.
...
| Tool                         | What it does                                                                                                      |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `sandbox_create`             | Create a new sandbox; returns its `id`. Accepts `secrets`, egress rules, and `preview_access`.                    |
| `sandbox_update`             | Change metadata, egress rules, lifecycle windows, or `preview_access`.                                            |
...
| `sandbox_preview_url`        | Publish a port and return a clean public URL or an expiring private signed URL.                                   |
| `sandbox_network_log`        | Audit a sandbox's outbound connections (host, verdict, bytes) without resuming it.                                |
...
- **Egress is controllable.** `allow_out` (domain patterns, IPs, or CIDRs) adds allowed destinations; `deny_out` (IPs or CIDRs, no domains) blocks them. `allow_out` alone does **not** lock a sandbox down — for a strict allowlist, combine it with `deny_out: ["0.0.0.0/0"]` (deny all, then allow the listed destinations). If the allowlist has domain rules or the workload resolves hostnames, also allow the sandbox's DNS resolvers, `1.1.1.1/32` and `8.8.8.8/32`, since deny-all blocks DNS too; leave them out of an IP-only allowlist, where an allowed resolver is only an extra outbound channel. Set these on `sandbox_create` or `sandbox_update`, and audit what a sandbox actually reached with `sandbox_network_log`.
...
**Ports.** New MCP sandboxes use `public` as the default access for newly
published ports; only explicitly published ports are reachable. Pass
`preview_access: "private"` to `sandbox_create` (or `sandbox_update`) to change
the default for future ports. Existing ports retain their own mode. Start the
server with `sandbox_exec`, then call `sandbox_preview_url`; the tool
idempotently publishes that one port and uses the returned port mode to return
either a clean public URL or an expiring private signed URL. Private links
default to one hour; set `expires_in_seconds` to a value from 1 through 604800
seconds. See [Preview URLs](/sandbox/preview-urls).
